...
type AssignLineItemIndexV2 struct {
	assign.Schema
	Params        payload.LineItemIndexV2
	Domain        []model.SearchLineItemV2
	Format        []model.SearchLineItemV2
	Size          []model.SearchLineItemV2
	AdTag         []model.SearchLineItemV2
	Country       []model.SearchLineItemV2
	Device        []model.SearchLineItemV2
	DomainSearch  []model.SearchLineItemV2
	FormatSearch  []model.SearchLineItemV2
	SizeSearch    []model.SearchLineItemV2
	AdTagSearch   []model.SearchLineItemV2
	CountrySearch []model.SearchLineItemV2
	DeviceSearch  []model.SearchLineItemV2
}

func (t *LineItemV2) Index(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2)
	if !isAccept {
	...
	assigns := AssignLineItemIndexV2{Schema: assign.Get(ctx)}
	assigns.Params = params
	assigns.Domain = new(model.LineItemV2).GetFilter("domain", userLogin.Id, params.Domain)
	assigns.Format = new(model.LineItemV2).GetFilter("format", userLogin.Id, params.AdFormat)
	assigns.Size = new(model.LineItemV2).GetFilter("size", userLogin.Id, params.AdSize)
	assigns.AdTag = new(model.LineItemV2).GetFilter("adtag", userLogin.Id, params.AdTag)
	assigns.Country = new(model.LineItemV2).GetFilter("country", userLogin.Id, params.Country)
	assigns.Device = new(model.LineItemV2).GetFilter("device", userLogin.Id, params.Device)
	assigns.Title = config.TitleWithPrefix("Line Item")
	return ctx.Render("line-item-v2/index", assigns, view.LAYOUTMain)
}

func (t *LineItemV2) Filter(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2)
	if !isAccept {
	...
	// Get payload post
	var inputs payload.LineItemFilterPayloadV2
	if err := ctx.BodyParser(&inputs); err != nil {
		return err
	}
	// Get data from model
	dataTable, err := new(model.LineItemV2).GetByFilters(&inputs, userLogin, GetLang(ctx))
	if err != nil {
		return err
	}
	// Print Data to JSON
...
func (t *LineItemV2) Choose(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2Choose)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	assigns := AssignHome{Schema: assign.Get(ctx)}
	assigns.Title = config.TitleWithPrefix("Choose a bidder")
	return ctx.Render("line-item-v2/choose", assigns, view.LAYOUTMain)
}

type AssignBidderAddV2 struct {
	assign.Schema
...
func (t *LineItemV2) Add(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2Add)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	collapse := new(model.LineItemV2).GetListBoxCollapse(userLogin.Id, 0, "line-item-v2", "add")
	assigns := AssignBidderAddV2{Schema: assign.Get(ctx)}
	...
	return ctx.Render("line-item-v2/add", assigns, view.LAYOUTMain)
}

type AssignBidderEditV2 struct {
	assign.Schema
	Row          model.LineItemRecordV2
	BidderParams []payload.BidderInfoV2
	Tabs         []helpers.TabInfoStruct
	TabsJS       string
...
func (t *LineItemV2) Edit(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2Edit)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	assigns := AssignBidderEditV2{Schema: assign.Get(ctx)}
	id := ctx.Query("id")
	...
	var row model.LineItemRecordV2
	// Xử lý cho trang view và edit
	if assigns.Uri == config.URILineItemV2Edit {
		row, _ = new(model.LineItemV2).GetById(idLineItem, userLogin.Id)
		if row.Id == 0 || row.IsLock != mysql.TYPEIsLockTypeUnlock {
			return ctx.SendStatus(fiber.StatusNotFound)
		}
	} else if assigns.Uri == config.URILineItemV2View {
		row, _ = new(model.LineItemV2).GetById(idLineItem, userLogin.Id)
	...
	collapse := new(model.LineItemV2).GetListBoxCollapse(userLogin.Id, idLineItem, "line-item-v2", "edit")
	assigns.ListBoxCollapse = collapse
	//Parse json param from record
	var jsonBidderInfos []payload.BidderInfoV2
	var bidderInfos []model.LineItemBidderInfoRecordV2
	mapBidderIndex := make(map[int64]int)
	bidderInfos = new(model.LineItemBidderInfoV2).GetBidderInfoByLineItem(row.Id)
	for _, bidderInfo := range bidderInfos {
		var bidderParams []payload.BidderParamV2
		params := new(model.LineItemBidderParamsV2).GetParams(bidderInfo.Id)
		for _, param := range params {
			...
			bidderParams = append(bidderParams, payload.BidderParamV2{
				Param:      param,
		...
		jsonBidderInfos = append(jsonBidderInfos, payload.BidderInfoV2{
	...
	targets := new(model.TargetV2).GetTargetLineItem(idLineItem)
	...
	if row.AutoCreate == 1 { // 1 - là trường hợp auto create dùng layout view riêng
		...
		return ctx.Render("line-item-v2/view", assigns, view.LAYOUTMain)
	} else {
		assigns.Title = config.TitleWithPrefix("Edit Line Item " + row.Name)
		return ctx.Render("line-item-v2/edit", assigns, view.LAYOUTMain)
	}
}

type AssignBidderCreateV2 struct {
	assign.Schema
	Countries []model.CountryRecord
	AdSizes   []model.AdSizeRecord
	AdFormats []model.AdTypeRecord
...
func (t *LineItemV2) AddPost(ctx *fiber.Ctx) error {
	// Get use login
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2Add)
	...
	// Get Post Data
	inputs := payload.LineItemAddV2{}
	if err := json.Unmarshal(ctx.Body(), &inputs); err != nil {
		return err
	}
	// Handle
	response := ajax.Responses{}
	newLineItem, errs := new(model.LineItemV2).AddLineItem(inputs, userLogin, userAdmin)
	if len(errs) > 0 {
...
func (t *LineItemV2) EditPost(ctx *fiber.Ctx) error {
	// Get use login
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2Edit)
	...
	// Handle
	response := ajax.Responses{}
	newLineItem, errs := new(model.LineItemV2).UpdateLineItem(inputs, userLogin, userAdmin)
	if len(errs) > 0 {
		response.Status = ajax.ERROR
		response.Errors = errs
	} else {
...
func (t *LineItemV2) Delete(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2Delete)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	...
	notify := new(model.LineItemV2).Delete(inputs.Id, userLogin.Id, userAdmin, GetLang(ctx))
	return ctx.JSON(notify)
}

type DataLoadMoreBidderV2 struct {
	Key             int                  `json:"key"`
	Search          string               `json:"search"`
	BiddersSelected []payload.LineItemV2 `json:"bidders_selected"`
}

type TemplateLoadMoreBidderV2 struct {
...
type BidderParamV2 struct {
	...
	ParamValue       []payload.BidderInfoV2
	Link             string
}

func (t *LineItemV2) BidderParam(ctx *fiber.Ctx) (err error) {
	// Get use login
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2LoadParam)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	...
	data := htmlblock.Render("line-item-v2/block_html/bidder_param_block.gohtml", bidderParam).String()
	var moreData payload.LoadMoreData
	moreData.Data = data
	return ctx.JSON(moreData)
}

func (t *LineItemV2) SearchDomain(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URISearchDomainV2)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	search := ctx.Query("q")
	data := new(model.LineItemV2).GetInventoryByName(search, userLogin.Id)
	return ctx.JSON(data)
}

func (t *LineItemV2) SearchAdFormat(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URISearchAdFormatV2)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	search := ctx.Query("q")
	data := new(model.LineItemV2).GetAdFormatByName(search)
	return ctx.JSON(data)
}

func (t *LineItemV2) SearchAdSize(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URISearchAdSizeV2)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	search := ctx.Query("q")
	data := new(model.LineItemV2).GetAdSizeByName(search)
	return ctx.JSON(data)
}

func (t *LineItemV2) SearchAdTag(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URISearchAdTagV2)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	search := ctx.Query("q")
	data := new(model.LineItemV2).GetAdTagByName(search, userLogin.Id)
	return ctx.JSON(data)
}

func (t *LineItemV2) SearchDevice(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URISearchDeviceV2)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	search := ctx.Query("q")
	data := new(model.LineItemV2).GetDeviceByName(search)
	return ctx.JSON(data)
}

func (t *LineItemV2) SearchCountry(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URISearchCountryV2)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	search := ctx.Query("q")
	data := new(model.LineItemV2).GetCountryByName(search)
	return ctx.JSON(data)
}

func (t *LineItemV2) Collapse(ctx *fiber.Ctx) (err error) {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2Collapse)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	...
	inputs.PageCollapse = "line-item-v2"
...
func (t *LineItemV2) CheckParam(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2CheckParam)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	name := ctx.Query("name")
	param := ctx.Query("param")
	check := new(model.LineItemV2).CheckParamRequired(name, param)
	return ctx.JSON(fiber.Map{
		"required": check,
	})
}

func (t *LineItemV2) ListLinkedGam(ctx *fiber.Ctx) error {
	// Get use login
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URILineItemV2ListLinkedGam)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
...
type ParamBidderPBV2 struct {
	Bidder model.PbBidderRecord
	Params []model.PbBidderParamRecord
}

func (t *LineItemV2) AddParamBidder(ctx *fiber.Ctx) error {
	userLogin := GetUserLogin(ctx)
	userAdmin := GetUserAdmin(ctx)
	isAccept := new(model.User).CheckUserLogin(userLogin, userAdmin, config.URIAddParamBidderV2)
	if !isAccept {
		return ctx.SendStatus(fiber.StatusNotFound)
	}
	demand := ctx.Query("demand")

	var paramBidderPB ParamBidderPBV2
	...
	data := htmlblock.Render("line-item-v2/block_html/add_param_bidder.gohtml", paramBidderPB).String()
	return ctx.JSON(data)
}
