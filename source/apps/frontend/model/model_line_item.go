...
func (LineItemRecord) TableName() string {
	return mysql.Tables.LineItem
}

type ListOfLineItem struct {
	Id   int64
	List []string
}
...
func (LineItem) GetById(id, userId int64) (row LineItemRecord, err error) {
	err = mysql.Client.
		//Debug().
	...
	row.TableLineItem.GetRls()
	return
}

func (LineItem) GetByUser(userId int64) (records []LineItemRecord) {
	mysql.Client.Where("status = 1 and user_id = ?", userId).Find(&records)
	return
}

func (rec *LineItemRecord) makeRecord(payload payload.LineItemAdd, userId int64) (err error) {
	rec.Id = payload.Id
	rec.UserId = userId
...
func (t *LineItem) GetLineSystemApacdex(inventoryId int64) (record LineItemRecord, err error) {
	err = mysql.Client.Where("apd_inventory = ? && user_id = 0", inventoryId).Find(&record).Error
	return
}

func (t *LineItem) AutoCreateLineSystemApacdex(inventory InventoryRecord, bidder BidderRecord) (errs []ajax.Error) {
	...
	lineItem := LineItemRecord{mysql.TableLineItem{
		UserId:       0,
	...
	bidderInfo := new(LineItemBidderInfo).CreateBidderInfo(LineItemBidderInfoRecord{mysql.TableLineItemBidderInfo{
	...
	_ = new(Target).DeleteTargetInventory(TargetRecord{mysql.TableTarget{
		LineItemId: lineItem.Id,
	}})
	// Tạo target cho inventory
	mysql.Client.Create(&TargetRecord{mysql.TableTarget{
		UserId:      0,
		LineItemId:  lineItem.Id,
...
func (t *LineItem) UpdateLineSystemApacdex(lineItem LineItemRecord, bidderId int64) (errs []ajax.Error) {
	...
	if status == mysql.TYPEStatusConnectionDemandLive {
		...
		if lineItem.Status == mysql.Off {
			mysql.Client.Model(LineItemRecord{}).Where("id = ?", lineItem.Id).Update("status", mysql.On)
		}
		...
		mysql.Client.Model(LineItemRecord{}).Where("id = ?", lineItem.Id).Update("status", mysql.Off)
	}
	return
}

func (t *LineItem) AddLineItem(inputs payload.LineItemAdd, user UserRecord, userAdmin UserRecord) (record LineItemRecord, errs []ajax.Error) {
	// Validate inputs
	...
	recordNew, _ := new(LineItem).GetById(record.Id, record.UserId)
...
func (t *LineItem) UpdateLineItem(inputs payload.LineItemAdd, user UserRecord, userAdmin UserRecord) (recordNew LineItemRecord, errs []ajax.Error) {
	lang := lang.Translate
	...
	//		status = mysql.Off
	//	}
	//	mysql.Client.Model(&LineItemRecord{}).Where("id = ?", inputs.Id).Update("status", status)
	...
	new(LineItemBidderInfo).DeleteBidderInfo(recordNew.Id)
	new(LineItemBidderParams).DeleteByLineItemId(recordNew.Id)
	...
	if recordNew.ServerType == 2 {
		new(LineItem).PushToQueueWorkerLineItemDfp(recordNew.Id)
	}
	// Get all lại data recordNew
	recordNew, _ = new(LineItem).GetById(recordNew.Id, recordNew.UserId)
...
func (t *LineItem) Validate(inputs payload.LineItemAdd, user UserRecord) (errs []ajax.Error) {
...
func (t *LineItem) ValidateAdsenseAdSlot(inputs payload.LineItemAdd) (errs []ajax.Error) {
...
func (t *LineItem) ValidateBidder(inputs payload.LineItemAdd, user UserRecord) (errs []ajax.Error) {
...
func (t *LineItem) validateValueOfType(paramName string, idParam string, typ string, value string) (errs []ajax.Error) {
	switch typ {
	case "int":
		if value != "" {
...
func (t *LineItem) Delete(id, userId int64, userAdmin UserRecord, lang lang.Translation) fiber.Map {
	record, _ := new(LineItem).GetById(id, userId)
...
func (t *LineItem) GetByFilters(inputs *payload.LineItemFilterPayload, user UserRecord, lang lang.Translation) (response datatable.Response, err error) {
...
type BidderRecordDatatable struct {
	LineItemRecord
...
func (t *LineItem) MakeResponseDatatable(bidders []LineItemRecord, userId int64) (records []BidderRecordDatatable) {
	for _, bidder := range bidders {
		...
		rec := BidderRecordDatatable{
			...
			Name:           htmlblock.Render("line-item/index/block.name.gohtml", target).String(),
			Target:         htmlblock.Render("line-item/index/block.target-button.gohtml", target).String(),
			Status:         htmlblock.Render("line-item/index/block.status.gohtml", bidder).String(),
			Type:           htmlblock.Render("line-item/index/block.type.gohtml", bidder).String(),
			Rate:           htmlblock.Render("line-item/index/block.rate.gohtml", bidder).String(),
			Action:         htmlblock.Render("line-item/index/block.action.gohtml", bidder).String(),
		}
...
func (t *LineItem) SetFilterStatus(inputs *payload.LineItemFilterPayload) func(db *gorm.DB) *gorm.DB {
...
func (t *LineItem) setFilterType(inputs *payload.LineItemFilterPayload) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
...
func (t *LineItem) setFilterSearch(inputs *payload.LineItemFilterPayload) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		var flag bool
...
func (t *LineItem) setOrder(inputs *payload.LineItemFilterPayload) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		var orders []string
...
func (t *LineItem) setFilterTarget(inputs *payload.LineItemFilterPayload, user UserRecord) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		...
		return db.Where("line_item.id in ?", listLineItemFilter)
	}
}

func (t *LineItem) getTarget(lineItem LineItemRecord, userId int64) Targets {
...
func (t *LineItem) CreateAdsenseAdSlot(lineItemId int64, inputs payload.LineItemAdd) (errs []ajax.Error) {
...
func (t *LineItem) CreateBidderInfo(lineItemId int64, inputs payload.LineItemAdd) (err error) {
	for _, bidderInfo := range inputs.BidderParams {
...
func (t *LineItem) CreateTarget(lineItemId, userId int64, inputs payload.LineItemAdd) (err error) {
	all := int64(-1)
	...
	for _, inventory := range inputs.ListAdInventory {
		...
		err := mysql.Client.Table(mysql.Tables.Target).FirstOrCreate(&recordTarget, TargetRecord{mysql.TableTarget{
			UserId:      userId,
			LineItemId:  lineItemId,
			InventoryId: inventory.Id,
		}}).Error
...
func (t *LineItem) GetInventoryByName(search string, userId int64) (rows []SearchLineItem) {
	//q := "%" + search + "%"
...
func (t *LineItem) GetOfUserById(id, userId int64) (row LineItemRecord) {
...
func (t *LineItem) InArray(array []SearchLineItem, id int64) (index int, flag bool) {
	for i, v := range array {
		if v.Id == id {
			return i, true
		}
...
func (t *LineItem) GetFilter(option string, userId int64, searched []string) (rows []SearchLineItem) {
	switch option {
	case "domain":
...
func (t *LineItem) GetListBoxCollapse(userId, lineItemId int64, page, typ string) (list []string) {
	switch typ {
	case "add":
...
func (t *LineItem) Count(id string, listId []string) bool {
	var count int
...
func (t *LineItem) CheckParamRequired(name, param string) (check bool) {
...
func (t *LineItem) PushToQueueWorkerLineItemDfp(lineItemId int64) {
	//Push to queue worker với lineId và type là google
	mysql.Client.Model(LineItemRecord{}).Where("id = ? and server_type = 2", lineItemId).Update("push_line_item_dfp", 1)
	return
}

func (t *LineItem) BuildLineNameAdsenseDisplay(inventoryName string, account string) string {
	return "[Default] - " + inventoryName + " - Adsense(" + account + ") - Display"
}

func (t *LineItem) BuildLineNameAdsenseVideo(inventoryName string, account string) string {
	return "[Default] - " + inventoryName + " - Adsense(" + account + ") - Video"
}

func (t *LineItem) GetByName(name string) (record LineItemRecord) {
	mysql.Client.Where("name = ?", name).Find(&record)
	return
}

func (t *LineItem) ScanLineItemPrebidOld() {
	//var userLogin UserRecord
	var records []LineItemRecord
	mysql.Client.
		Where("server_type = ? and user_id != 0", mysql.TYPEServerTypePrebid).
		//Where("id = 19541").
		Find(&records)

	for _, record := range records {
		record.GetRls()
		if len(record.BidderInfo) == 0 {
			continue
		}

		var targetDomain []int64
		var targetFormat []int64
		var targetSize []int64
		var targetAdTag []int64
		var targetDevice []int64
		var targetGeo []int64
		for _, target := range record.Targets {
			if target.InventoryId != 0 {
				targetDomain = append(targetDomain, target.InventoryId)
			}
			if target.AdFormatId != 0 {
				targetFormat = append(targetFormat, target.AdFormatId)
			}
			if target.AdSizeId != 0 {
				targetSize = append(targetSize, target.AdSizeId)
			}
			if target.TagId != 0 {
				targetAdTag = append(targetAdTag, target.TagId)
			}
			if target.DeviceId != 0 {
				targetDevice = append(targetDevice, target.DeviceId)
			}
			if target.GeoId != 0 {
				targetGeo = append(targetGeo, target.GeoId)
			}
		}

		fmt.Printf("targetDomain: %+v \n", targetDomain)
		fmt.Printf("targetFormat: %+v \n", targetFormat)
		fmt.Printf("targetFormat: %+v \n", utility.InArray(-1.0, targetFormat, false))
		fmt.Printf("targetSize: %+v \n", targetSize)
		fmt.Printf("targetAdTag: %+v \n", targetAdTag)
		fmt.Printf("targetDevice: %+v \n", targetDevice)
		fmt.Printf("targetGeo: %+v \n", targetGeo)
		var isBannerConfig bool
		var isVideoInstreamConfig bool
		var isVideoOutstreamConfig bool
		var isNativeConfig bool

		if utility.InInt64Array(-1, targetFormat) {
			isBannerConfig = true
			isVideoInstreamConfig = true
			isVideoOutstreamConfig = true
			isNativeConfig = true
		}

		if utility.InInt64Array(int64(mysql.AdTypeDisplay), targetFormat) || utility.InInt64Array(int64(mysql.AdTypeRelatedZone), targetFormat) {
			// Nếu size target all hoặc là  "300x250", "336x280", "970x250", "300x600" thì mở video out stream config
			//if utility.InInt64Array(-1, targetSize) || utility.InInt64Array(14, targetSize) || utility.InInt64Array(8, targetSize) || utility.InInt64Array(4, targetSize) || utility.InInt64Array(1, targetSize) {
			//	isVideoOutstreamConfig = true
			//}
			isBannerConfig = true
		}

		if utility.InInt64Array(int64(mysql.AdTypeInterstitial), targetFormat) {
			//isVideoInstreamConfig = true
			//isVideoOutstreamConfig = true
			isBannerConfig = true
		}

		if utility.InInt64Array(int64(mysql.AdTypeStickyBanner), targetFormat) {
			isBannerConfig = true
		}

		if utility.InInt64Array(int64(mysql.AdTypeNative), targetFormat) {
			isNativeConfig = true
		}

		if utility.InInt64Array(int64(mysql.AdTypeInstream), targetFormat) {
			isVideoInstreamConfig = true
			//isVideoOutstreamConfig = true
			//isBannerConfig = true
		}

		if utility.InInt64Array(int64(mysql.AdTypeOutstream), targetFormat) {
			isVideoOutstreamConfig = true
		}

		var selectAccount []int64
		selectAccount = append(selectAccount, record.BidderGoogle.Id)

		var bidderInfoBanner []payload.BidderInfo
		var bidderInfoVideoInstream []payload.BidderInfo
		var bidderInfoVideoOutstream []payload.BidderInfo
		var bidderInfoNative []payload.BidderInfo
		for _, info := range record.BidderInfo {
			var bidderParams []payload.BidderParam

			paramBanners := make(map[string]payload.ParamDetail)
			paramVideoInstreams := make(map[string]payload.ParamDetail)
			paramVideoOutstreams := make(map[string]payload.ParamDetail)
			paramNatives := make(map[string]payload.ParamDetail)
			for _, param := range info.Params {
				paramBanners[param.Name] = payload.ParamDetail{
					Value:      param.Value,
					Type:       param.Type,
					IsAddParam: 2,
				}
				paramVideoInstreams[param.Name] = payload.ParamDetail{
					Value:      param.Value,
					Type:       param.Type,
					IsAddParam: 2,
				}
				paramVideoOutstreams[param.Name] = payload.ParamDetail{
					Value:      param.Value,
					Type:       param.Type,
					IsAddParam: 2,
				}
				paramNatives[param.Name] = payload.ParamDetail{
					Value:      param.Value,
					Type:       param.Type,
					IsAddParam: 2,
				}
			...
			bidder := new(Bidder).GetByIdNoCheckUser(info.BidderId)
			bidderInfoBanner = append(bidderInfoBanner, payload.BidderInfo{
				BidderId:       info.BidderId,
				BidderName:     info.Name,
				ConfigType:     mysql.TYPEConfigTypeBanner,
				BidderType:     info.BidderType,
				BidderIndex:    0,
				BidderTemplate: bidder.BidderTemplateId,
				Params:         paramBanners,
				BidderParams:   bidderParams,
				Link:           new(PbBidder).GetLinkByBidderCode(info.Name),
			})
			bidderInfoVideoInstream = append(bidderInfoVideoInstream, payload.BidderInfo{
				BidderId:       info.BidderId,
				BidderName:     info.Name,
				ConfigType:     mysql.TYPEConfigTypeVideoInstream,
				BidderType:     info.BidderType,
				BidderIndex:    0,
				BidderTemplate: bidder.BidderTemplateId,
				Params:         paramVideoInstreams,
				BidderParams:   bidderParams,
				Link:           new(PbBidder).GetLinkByBidderCode(info.Name),
			})
			bidderInfoVideoOutstream = append(bidderInfoVideoOutstream, payload.BidderInfo{
				BidderId:       info.BidderId,
				BidderName:     info.Name,
				ConfigType:     mysql.TYPEConfigTypeVideoOutStream,
				BidderType:     info.BidderType,
				BidderIndex:    0,
				BidderTemplate: bidder.BidderTemplateId,
				Params:         paramVideoOutstreams,
				BidderParams:   bidderParams,
				Link:           new(PbBidder).GetLinkByBidderCode(info.Name),
			})
			bidderInfoNative = append(bidderInfoNative, payload.BidderInfo{
				BidderId:       info.BidderId,
				BidderName:     info.Name,
				ConfigType:     mysql.TYPEConfigTypeNative,
				BidderType:     info.BidderType,
				BidderIndex:    0,
				BidderTemplate: bidder.BidderTemplateId,
				Params:         paramNatives,
		...
		var bidderInfos []payload.BidderInfo
		var listInventory, listAdFormat, listAdSize, listAdtag, listDevice, listGeo []payload.ListTarget
		for _, inventoryID := range targetDomain {
			if inventoryID == -1 {
				continue
			}
			inventory, err := new(Inventory).GetByIdSystem(inventoryID)
			if err != nil {
				inventory.Id = inventoryID
			}
			listInventory = append(listInventory, payload.ListTarget{
				Id:   inventory.Id,
				Name: inventory.Name,
			})
		}

		for _, formatID := range targetFormat {
			if formatID == -1 {
				continue
			}
			adType := new(AdType).GetById(formatID)
			listAdFormat = append(listAdFormat, payload.ListTarget{
				Id:   adType.Id,
				Name: adType.Name,
			})
		}

		for _, sizeID := range targetSize {
			if sizeID == -1 {
				continue
			}
			adSize := new(AdSize).GetById(sizeID)
			listAdSize = append(listAdSize, payload.ListTarget{
				Id:   adSize.Id,
				Name: adSize.Name,
			})
		}

		for _, tagID := range targetAdTag {
			if tagID == -1 {
				continue
			}
			tag, _ := new(AdTag).GetDetail(tagID)
			listAdtag = append(listAdtag, payload.ListTarget{
				Id:   tag.Id,
				Name: tag.Name,
			})
		}

		for _, deviceID := range targetDevice {
			if deviceID == -1 {
				continue
			}
			device := new(Device).GetById(deviceID)
			listDevice = append(listDevice, payload.ListTarget{
				Id:   device.Id,
				Name: device.Name,
			})
		}

		for _, geoID := range targetGeo {
			if geoID == -1 {
				continue
			}
			geo := new(Country).GetById(geoID)
			listGeo = append(listGeo, payload.ListTarget{
				Id:   geo.Id,
				Name: geo.Name,
			})
		}
		...
		if isBannerConfig {
			bidderInfos = append(bidderInfos, bidderInfoBanner...)
		}

		if isVideoInstreamConfig {
			bidderInfos = append(bidderInfos, bidderInfoVideoInstream...)
		}

		if isVideoOutstreamConfig {
			bidderInfos = append(bidderInfos, bidderInfoVideoOutstream...)
		}

		if isNativeConfig {
			bidderInfos = append(bidderInfos, bidderInfoNative...)
		}

		if len(bidderInfos) == 0 {
			fmt.Printf("record: %+v \n", record)
			fmt.Printf("isBannerConfig: %+v \n", isBannerConfig)
			fmt.Printf("isVideoInstreamConfig: %+v \n", isVideoInstreamConfig)
			fmt.Printf("isVideoOutstreamConfig: %+v \n", isVideoOutstreamConfig)
			fmt.Printf("isNativeConfig: %+v \n", isNativeConfig)
			logger.Error("bidder info is empty")
			continue
		}

		//return
		recordNew, errs := new(LineItem).UpdateLineItem(payload.LineItemAdd{
			Id:              record.Id,
			Name:            record.Name,
			...
			GamLineItemType: record.GamLineItemType,
			LinkedGam:       record.LinkedGam,
			Status:          record.Status.String(),
			BidderParams:    bidderInfos,
			BidderVastUrl:   record.VastUrl,
			BidderAdTag:     record.AdTag,
			Priority:        record.Priority,
			ListAdFormat:    listAdFormat,
			ListAdSize:      listAdSize,
			ListAdTag:       listAdtag,
			ListAdInventory: listInventory,
			ListGeo:         listGeo,
			ListDevice:      listDevice,
		}, UserRecord{mysql.TableUser{Id: record.UserId}}, UserRecord{mysql.TableUser{Id: 169}})
		if len(errs) > 0 {
			fmt.Printf("Line Item: %+v --Error:%+v \n", recordNew, errs)
		}
	}
}
