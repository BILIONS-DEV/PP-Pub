...
$(document).ready(function () {
    ...
    new submitFormCreate("formTemplate", added, "/player/template/edit");
    firstLoad();
    selectTab();
    selectPlayerType();
    selectPlayerLayout();
...
function checkPoweredByValueimpression() {
    let adType = $("#type").val();
    if (adType === "4") {
...
function checkValueimpressionLogo() {
    let adType = $("#type").val();
    let layout = $("#player_layout").val();
    let videoType = getVideoType(adType, layout);

    if (videoType === videoTypeInstream) {
...
function checkShowColumnsPosition() {
    ...
    if (videoType === videoTypeInstream && (layout === "6" || layout === "7")) {
...
function checkShowColumnsNumber() {
    ...
    if (videoType === videoTypeInstream && layout === "6") {
        box.removeClass("d-none");
        if (first === false) {
...
function checkDisableLogoConfig() {
    ...
    if (videoType === videoTypeInstream || adType === "4") {
...
function checkDisableColor() {
    ...
    } else if (videoType === videoTypeOutstream) {
        controlsColor.removeClass("d-none");
        controlsColor = $("#control_color");
        controlsColor.addClass("disabled");
        controlsColor.prop("disabled", true);
...
function checkDisableDisplayOption() {
    ...
    if (((videoType === videoTypeInstream || adType === "4") && showMainTitleElement.is(":checked"))) {
    ...
    if (((videoType === videoTypeInstream || adType === "4") && ShowSubTitle.is(":checked"))) {
...
function checkShowTabCustomize() {
    let adType = $("#type").val();
    let layout = $("#player_layout").val();
    let videoType = getVideoType(adType, layout)
    if (videoType === videoTypeOutstream) {
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if (json.template.adType === "Pin Zone") {
                viAPItag.initPinzoneTemplate(json);
...
const videoTypeInstream = "instream";
const videoTypeOutstream = "outstream";

function getVideoType(adType, layout) {
    if (adType === "9" && ["1", "2", "3", "4", "5", "6", "7", "8"].includes(layout)) {
        return videoTypeInstream;
    } else if (adType === "9" && layout === "9") {
        return videoTypeOutstream
    } else {
        return "";
    }
}