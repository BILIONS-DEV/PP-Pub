import {duration} from "../../../../../www/themes/atlantis/js/plugin/moment/moment";

const formID = "#formFilter"
const tableID = "#tableResponse"
const filterURL = "/channels";
const currentURL = "/channels";
let firstLoad = true;
...
$(document).ready(function () {
    $(tableID).on("click", "tbody td div.btn-group a.remove", function (e) {
        var id = $(this).data("id")
        var isCheck = confirm("You are about to delete the entire channel, are you sure you wish to proceed?");
        if (isCheck) {
            Delete(id)
        }
    });
...
function GetTable(isClickForm = false) {
    ...
    let setting = {
        ...
        drawCallback: function () {
            ...
            // $(".dataTables_paginate > ul.pagination > li.active > a").addClass('bg-green-400 border-success').css("color", "#111111");
...
function Delete(id) {
    ...
    $.ajax({
        ...
        switch (result.status) {
            case "success":
                NoticeSuccess("Playlist has been removed successfully")
                GetTable(false)
                break
            case "err":
...
function LoadVideoSource(element) {
    ...
    var url = domainUpload + element.data("url")
    if (Hls.isSupported()) {
...
function ShowVideo(element) {
    var id = element.data("id")
    var modal = document.getElementsByClassName(`link_video_${id}`);
    $(`.link_video_${id}`).css("display", "block")
    $(`span#${id}`).on("click", function () {
        $(`.link_video_${id}`).css("display", "block")
    })
    $("span.close").on("click", function () {
        $(`.link_video_${id}`).css("display", "none")
    })
