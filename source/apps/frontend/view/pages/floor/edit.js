...
$(document).ready(function () {
    $(".floor-tab").on("click", ".nav-link", function () {
        $(".floor-tab").find(".nav-link").removeClass("pp-4");
        $(this).addClass("pp-4");
        var tab = $(this).attr("data-tab");
        if (tab != "1") {
            $(this).addClass("at-1");
        } else {
            $(".floor-tab").find('.at-1').removeClass('at-1');
        }
    });
    ...
    //Submit
    new SubmitFormCreate("AddBidder", Added);
...
function changeFloorType() {
    var floor_type = $("#floor_type").val();
    floor_type = parseInt(floor_type);
    if (floor_type === 1) {
        $(".box-floor-value").addClass("d-none");
        $(".floor_type_hard").addClass("d-none");
        $(".floor_type_dynamic").removeClass("d-none");
    } else {
        $(".box-floor-value").removeClass("d-none");
        $(".floor_type_dynamic").addClass("d-none");
        $(".floor_type_hard").removeClass("d-none");
    }
    $("#floor_type").change(function () {
        var floor_type = $(this).val();
        floor_type = parseInt(floor_type);
        if (floor_type === 1) {
            $(".box-floor-value").addClass("d-none");
            $(".floor_type_hard").addClass("d-none");
...
function Added(response, formElement) {
    switch (response.status) {
        case "error":
            ...
            break;
        case "success":
            NoticeSuccess("Floor has been updated successfully");
            break;
        default:
...
function SubmitFormCreate(formID, functionCallback, ajxURL = "") {
    ...
    function submit() {
        ...
        postData.floor_type = parseInt(postData.floor_type);

        if (postData.floor_type == 1) {
            postData.floor_value = 0;
        } else {
            if (postData.floor_value !== "") {
                postData.floor_value = parseFloat(postData.floor_value)
            } else {
                postData.floor_value = 0;
            }
        }
