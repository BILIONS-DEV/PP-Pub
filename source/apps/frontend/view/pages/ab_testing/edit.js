$(document).ready(function () {
    $(".ab-testing-tab").on("click", ".nav-link", function () {
        $(".ab-testing-tab").find(".nav-link").removeClass("pp-4");
        $(this).addClass("pp-4");
        var tab = $(this).attr("data-tab");
        if (tab != "1") {
            $(this).addClass("at-1");
        } else {
            $(".ab-testing-tab").find('.at-1').removeClass('at-1');
        }
    });
    ...
    firstLoadPage();
    SubmitFormCreate("submitAbTesting", Added, "/ab-testing/edit");
    selectTestType();
    dateTimePicker();
    // LoadHistory();
});
...
function firstLoadPage() {
    let elementClassTestTypeAuctionTimeOut = $(".test_type_auction_timeout");
    let elementClassTestTypeClientVsServer = $(".test_type_client_server");
    let elementClassTestTypeUserIdModule = $(".test_type_user_id_module");
    let elementClassTestTypeDynamicHardPriceFloor = $(".test_type_dynamic_hard_price_floor");
    let elementTabSetup = $("#tab-setup");
    let elementTabTarget = $("#tab-target");
    switch ($("#test_type").val()) {
        case "1": // Auction Timeout
            elementClassTestTypeClientVsServer.addClass("d-none");
            elementClassTestTypeUserIdModule.addClass("d-none");
            elementClassTestTypeDynamicHardPriceFloor.addClass("d-none");
            elementClassTestTypeAuctionTimeOut.removeClass("d-none");
            elementTabTarget.removeClass("d-none");
            break;
        case "2": // Client vs server
            elementClassTestTypeAuctionTimeOut.addClass("d-none");
            elementClassTestTypeUserIdModule.addClass("d-none");
            elementClassTestTypeDynamicHardPriceFloor.addClass("d-none");
            elementClassTestTypeClientVsServer.removeClass("d-none");
            elementTabTarget.removeClass("d-none");
            break;
        case "3": // User Id Module
            elementClassTestTypeAuctionTimeOut.addClass("d-none");
            elementClassTestTypeClientVsServer.addClass("d-none");
            elementClassTestTypeDynamicHardPriceFloor.addClass("d-none");
            elementClassTestTypeUserIdModule.removeClass("d-none");
            elementTabTarget.removeClass("d-none");
            break;
        case "4": // Dynamic vs Hard Price Floor
            elementClassTestTypeAuctionTimeOut.addClass("d-none");
            elementClassTestTypeClientVsServer.addClass("d-none");
            elementClassTestTypeUserIdModule.addClass("d-none");
            elementClassTestTypeDynamicHardPriceFloor.removeClass("d-none");
            elementTabSetup.click();
            elementTabTarget.addClass("d-none");
            break;
    }
}
...
function dateTimePicker() {
    let elementDateTimePicker = $('input.daterangepickers');
    elementDateTimePicker.daterangepicker({
    ...
    elementDateTimePicker.on('apply.daterangepicker', function (ev, picker) {
        $(this).val(picker.startDate.format('MM/DD/YYYY'));
    });
    elementDateTimePicker.on('cancel.daterangepicker', function (ev, picker) {
        $(this).val('');
    });
}
...
$(document).ready(function () {
    ...
    //Reload lại adtag theo filter sau khi setup
    ReloadAdTag()
});

function GetIsMoreData() {
    optionDomainsLoadMore.isMoreData = $("#is_more_inventory").val() === "true";
    optionDomainsLoadMore.lastPage = $("#last_page_domain").val() === "true";
    optionAdFormatsLoadMore.isMoreData = $("#is_more_adformat").val() === "true";
    optionAdFormatsLoadMore.lastPage = $("#last_page_format").val() === "true";
    optionAdSizesLoadMore.isMoreData = $("#is_more_adsize").val() === "true";
    optionAdSizesLoadMore.lastPage = $("#last_page_size").val() === "true";
    optionAdTagsLoadMore.isMoreData = $("#is_more_tag").val() === "true";
    optionAdTagsLoadMore.lastPage = $("#last_page_tag").val() === "true";
    optionGeographyLoadMore.isMoreData = $("#is_more_geography").val() === "true";
    optionGeographyLoadMore.lastPage = $("#last_page_geo").val() === "true";
    optionDeviceLoadMore.isMoreData = $("#is_more_device").val() === "true";
    optionDeviceLoadMore.lastPage = $("#last_page_device").val() === "true";
}
...
function setUpClickAndSearch(optionTarget) {
    ...
    // $(optionTarget.idSearch).on("keypress", function (e) {
    //     if (e.which === 13) {
    //         optionTarget.page = 1
...
function RemoveInclude(element, event, optionTarget) {
    ...
    // let divCountry = $(optionTarget.idSelect).find(`div[id = '${id}']`)
    // divCountry.attr("hidden", false)
...
//     $(optionTarget.idBox).append(`<div class="target-item border-bottom border-gray-50 ms-2 me-3 item_selected" id="${id}">
//         <div class="list-group list-group-flush my-n3">
//             <div class="list-group-item">
//                 <div class="d-flex flex-row align-items-center">
//                     <div class="col p-0">
...
function Added(response, formElement) {
    switch (response.status) {
        case "error":
            ...
            break
        case "success":
            NoticeSuccess("A/B Testing has been updated successfully")
            break
        default:
...
function SubmitFormCreate(formID, functionCallback, ajxURL = "") {
    ...
    formElement.find("input").on("edit", function (e) {
        let inputElement = $(this)
        if (inputElement.hasClass("is-invalid")) {
            inputElement.removeClass("is-invalid").next(".invalid-feedback").empty();
        }
    });
    formElement.find("textarea").on("edit", function (e) {
        let inputElement = $(this)
        if (inputElement.hasClass("is-invalid")) {
            inputElement.removeClass("is-invalid").next(".invalid-feedback").empty();
        }
    });
    formElement.find(".selectpicker").on("change", function (e) {
        let selectElement = $(this)
        if (selectElement.closest(".box-selectpicker").hasClass("is-invalid")) {
            selectElement.closest(".box-selectpicker").removeClass("is-invalid").next(".invalid-feedback").empty();
        }
    });
    ...
    function submit() {
        ...
        postData.listData = optionDomainsLoadMore.list_selected
        ...
        postData.test_type = parseInt(postData.test_type)
        postData.select_bidder = parseInt(postData.select_bidder)
        postData.select_user_id_module = parseInt(postData.select_user_id_module)
        postData.test_group_size = parseInt(postData.test_group_size)
        postData.dynamic_floor_price = parseInt(postData.dynamic_floor_price)
        postData.hard_price_floor = parseFloat(postData.hard_price_floor)
