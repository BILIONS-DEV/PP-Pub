...
const info = {
    ...
    formId: "EditBidder",
    urlLoadParam: "/line-item-v2/loadParam",
    urlSubmit: "/line-item-v2/edit",
    showHideSelectBidder: true,
    typeSelect: "selectpicker",
    ajaxAddParam: "/line-item-v2/addParamBidder",
    isCheckParamRequired: "/line-item-v2/checkParam",
    isEdit: true,
    topLoading: "15%",
    elementLoading: "#tab-prebid",
    colorLoading: "#adadad"
}
...
$(document).ready(function () {
    ...
    //xử lí các action prebid khi line đó là prebid
    if (!$(info.targetPrebid).hasClass("d-none")) {
        PrebidLoad();
    }
...
function PrebidLoad() {
    ...
    Prebid.handleDisabledTabs({ force: true }, fullInfo);

    Prebid.firstLoadBidderDisabled() // xử lí disabled các prebid có client nếu là s2s ko đc chọn sang client nữa
    ...
    Prebid.EventRemoveBidder(); //Remove Param -> OK

    Prebid.activeInFirstLoad();
}
...
function checkType(delay) {
    var modalHistory = $("#modal-history");
    var loadType = modalHistory.find("input[name='loadType']:checked").val();


    if (loadType == 'ChangesOnly') {
        if (delay == 'on') {
            modalHistory.find(".table-history").animate({ opacity: 0 }, 130)
            setTimeout(function () {
                modalHistory.find(".everything").addClass("d-none")
                modalHistory.find(".value-item").each(function () {
                    ...
                    var showItem = false
                    modalHistory.find(".value-" + id).each(function () {
                        if (!$(this).hasClass('d-none')) {
                            showItem = true
                        }
                    })
                    if (showItem) {
                        modalHistory.find(".item-" + id).removeClass('d-none')
                    } else {
                        modalHistory.find(".item-" + id).addClass('d-none')
                    }
                })
            }, 130)

            modalHistory.find(".table-history").animate({ opacity: 1 }, 130)
        } else {
            modalHistory.find(".everything").addClass("d-none")
            modalHistory.find(".table-history").animate({ opacity: 1 }, 130)
            modalHistory.find(".value-item").each(function () {
        ...
        if (delay == 'on') {
            modalHistory.find(".table-history").animate({ opacity: 0 }, 130)
            setTimeout(function () {
                modalHistory.find(".everything").removeClass("d-none")
                modalHistory.find(".title-item").removeClass("d-none")
            }, 130)
            modalHistory.find(".table-history").animate({ opacity: 1 }, 130)
        } else {
            modalHistory.find(".title-item").removeClass("d-none")
...
function getListLinkedGam(ppAdx = false, bidderAdxId) {
    $.ajax({
        url: "/line-item-v2/listLinkedGam",
        type: "GET",
        data: {
            ppAdx: ppAdx,
...
function Added(response) {
    switch (response.status) {
        ...
        case "success":
            ...
            // setTimeout(function () {
            //     window.location.href = "/line-item-v2";
            // }, 1000)
            break
        default:
...
function HandleCollapse() {
    ...
    let url = "/line-item-v2/collapse"
    $('#EditBidder').on('show.bs.collapse', '.collapse', function (e) {
        let box = e.target.id
        SendRequestShow(url, box, parseInt(params.id), "edit")
    });
    $('#EditBidder').on('hide.bs.collapse', '.collapse', function (e) {
        let box = e.target.id
        SendRequestHide(url, box, parseInt(params.id), "edit")
    });
}
