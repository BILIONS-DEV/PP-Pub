...
$(document).ready(function () {
    ...
    SubmitFormCreate("createContent", Added, "/content/add");
...
function initAdSchedule() {
    ...
    clickDeleteRoll();
    checkInputTimeBreak();
    checkSelectBreakMode();
}

function checkSelectBreakMode() {
    $('.box-config-adbreak').on("changed.bs.select", "select.break_mode", function () {
        let elementTimeBreak = $(this).closest(".box-adtag-url").find(".time_break");
        validateInputTimeBreak(elementTimeBreak);
    });
}
...
function checkInputTimeBreak() {
    $('.box-config-adbreak').on("input", ".time_break", function () {
        validateInputTimeBreak($(this));
    });
}

function validateInputTimeBreak(elementInputTimeBreak) {
    let elementBoxAdTagUrl = elementInputTimeBreak.closest(".box-adtag-url");
    let elementBoxRoll = elementBoxAdTagUrl.prev();
    let breakMode = elementBoxAdTagUrl.find(".break_mode").find("option:selected").val();
    let action = elementBoxRoll.attr("data-action");
    let htmlError = htmlBreakTimeError(breakMode, elementInputTimeBreak.val());
    if (elementInputTimeBreak.val().length === 0 || htmlError !== "") {
        if (action === "done") {
            elementBoxRoll.find(".box-step-roll").removeClass().addClass("box-step-roll").addClass("_error_z87zl_157");
            elementBoxRoll.find("._inactive_iyx7e_22").removeClass("_inactive_iyx7e_22").addClass("_error_iyx7e_31");
        }
        elementBoxAdTagUrl.prev().find(".btn-done-roll").addClass("-disabled").find("button").prop("disabled", true);
        elementBoxAdTagUrl.find(".box_time_break").addClass("-error -error-msg").find(".box_error_time_break").remove();
        elementBoxAdTagUrl.find(".box_time_break").append(htmlError);
    } else {
        if (action === "done") {
            elementBoxRoll.find(".box-step-roll").removeClass().addClass("box-step-roll").addClass("_editing_z87zl_152");
            elementBoxRoll.find("._error_iyx7e_31").removeClass("_error_iyx7e_31").addClass("_inactive_iyx7e_22");
        }
        elementBoxAdTagUrl.find(".box_time_break").removeClass("-error -error-msg").find(".box_error_time_break").remove();
...
function clickAddRoll() {
    $('.box-config-adbreak').on("click", ".box-btn-add", function () {
        ...
        elementBoxRoll.find(".text-adtag-url").addClass("d-none");
...
function clickCancelRoll() {
    $('.box-config-adbreak').on("click", ".btn-cancel-roll", function () {
        let elementBoxRoll = $(this).closest(".box-roll");
        elementBoxRoll.find(".text-adtag-url").removeClass("d-none");
        elementBoxRoll.find(".box-btn-edit").addClass("d-none");

        let elementBoxAdTagUrl = elementBoxRoll.next();
        let action = elementBoxRoll.attr("data-action");
        if (action === "add") {
            elementBoxRoll.find(".box-step-roll").removeClass().addClass("box-step-roll");
            ...
            elementBoxRoll.find(".box-step-roll").removeClass().addClass("box-step-roll").addClass("_active_z87zl_147");
            elementBoxRoll.find("._inactive_iyx7e_22").removeClass("_inactive_iyx7e_22").addClass("_active_iyx7e_28");
            elementBoxRoll.find("._error_iyx7e_31").removeClass("_error_iyx7e_31").addClass("_active_iyx7e_28");
            elementBoxRoll.find(".box-btn-action").removeClass("d-none");
            ...
            return ``;
                        ...
                        <div class="dropdown wui-more-menu -type-icon hydrated box-btn-action d-none">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownUserButton">
                                ${(() => {
        if (type === "midroll") {
            return `<a class="dropdown-item btn-edit-roll" type="button">Edit</a>`;
        } else {
            return ``;
        }
    })()}
...
function htmlBreakTimeError(mode, timeBreak) {
    let textError = "";
    switch (mode) {
        case "1":
            if (timeBreak === "") {
                textError = "Invalid number";
            } else if (parseInt(timeBreak) < 0) {
                textError = "Number must be equal to or greater than 0";
            }
            break;
        case "2":
            if (!timeBreak.match("/^\d{2,}:\d{2}:\d{2}$/")) {
                textError = "Please match format eg. 1:00:00.123";
            }
            break;
        case "3":
            if (timeBreak === "") {
                textError = "Invalid number";
            } else if (parseInt(timeBreak) < 0 || parseInt(timeBreak) > 100) {
                textError = "Number must be between 0 and 100";
            }
            break;
    }
    ...
    if (textError === "") {
        ...
        return `<div class="wui-input__error box_error_time_break" tabindex="1" title="${textError}">${textError}</div>`;
    }
}
    ...
    switch (typeUpload) {
        case"video":
            ...
            $(".preview-video").append(`
                     <video src="${blobURL}" style="max-height:535px;max-width: 870px; width: 100%;height: 100%;" controls="controls" type="video/mp4"></video>`);
            $.ajax({
        ...
        case "success":
            switch (typeUpload) {
                case "video":
                    ...
                    $(".select-thumb").parent().removeClass("cursor-not-allowed").removeClass('opacity-50');
                ...
                case "thumb":
                    ...
                    $(".preview-thumb").attr("src", domainUpload + response.data_object.thumb[0]).removeClass("d-none");
                    $("#thumb").removeClass("is-invalid").next(".invalid-feedback").empty();
                    $(".select-thumb").parent().removeClass("selected");
                    CheckImgSelected = false;
...
function Added(response, formElement) {
    switch (response.status) {
        case "error":
            ...
            break;
        case "success":
            NoticeSuccess("Content has been created successfully");
            setTimeout(function () {
