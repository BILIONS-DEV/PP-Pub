...
$val = getopt(null, ["refreshToken:", "networkId:", "networkName:", "currentCode:", "name:", "advertiserId:", "traffickerId:", "tagIds:", "orderId:", "type:", "bidderName:", "codeMoney:", "size:", "id:", "pubId:", "accountType:", "pubId:", "adSlotId:", "creativeId:", "lineItemId:"]);
if (empty($tokenTest)) {
    ...
    if (empty($val["networkName"])) {
        echo json_encode(["status" => false, "message" => "networkName is required"]);
        return;
    }
    if ($val["refreshToken"]) {
...
try {
    ...
    echo json_encode(["status" => false, "type" => "refresh_token", "message" => "Failed refresh token!", "log" => $exc->getMessage()]);
...
try {
    ...
    echo json_encode(["status" => false, "type" => "oauth2", "message" => "Failed oauth2!", "log" => $exc->getMessage()]);
    return;
}
